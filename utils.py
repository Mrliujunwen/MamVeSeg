...
def calculate_F1_score(prediction, label):
    ...
    dice = 2 * np.sum(intersection) / (np.sum(prediction) + np.sum(label) + 1e-7)
    return dice
...
def compute_class_sens(pred, label):
    """
    ...
    # 计算False Negative（FN）
    fn = np.sum(np.logical_and(np.logical_not(pred), label))

    sensitivity = tp / (tp + fn + 1e-7)

    return sensitivity

def compute_class_spec(pred, label):
    # 计算True Positive（TP）
    tp = np.sum(np.logical_and(pred, label))
